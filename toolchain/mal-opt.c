...
#include "src/wirble/mal/mal_ir.h"
...
{
  fprintf (stderr,
           "Usage: %s [--rules path] [--fixpoint] [--stats] input.wb\n",
           argv0);
}
...
main (int argc, char **argv)
{
  ...
  for (index = 1; index < argc; ++index)
    {
      if (strcmp (argv[index], "--rules") == 0)
        {
          ...
          options.mrsRulesPath = argv[++index];
          continue;
        }
      ...
      if (strcmp (argv[index], "--fixpoint") == 0)
        {
          continue;
        }
      if (argv[index][0] == '-')
  ...
  if (!wirbleBuildPipelineArtifacts (source, inputPath, &options, &artifacts,
    ...
    {
      fprintf (stderr, "mal-opt: %s\n",
               errorMessage == NULL ? "optimization failed" : errorMessage);
      goto cleanup;
    }
  if (showStats && artifacts.malModule != NULL)
    {
      fprintf (stdout, "mal-opt: functions=%u\n",
               artifacts.malModule->functionCount);
    }
  status = 0;
